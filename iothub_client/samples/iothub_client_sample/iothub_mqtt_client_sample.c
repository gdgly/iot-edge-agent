...
#define ENDPOINT "ghiot.cigem.cn"

#define USERNAME "384099"

#define PASSWORD "38f6a551132f623c48de"

#define CONNECTION_TYPE "TCP"

static const char* SUBTOPIC = "$creq/384099/+";
static const char* PUBTOPIC = "$dr";
...
void on_recv_callback(MQTT_MESSAGE_HANDLE msgHandle, void* context)
{
    ...
    sprintf(publishData, "%s%s", appMsg->message, "&reslut=succ&$cmd=reboo");

    publish_mqtt_message(clientHandle, PUBTOPIC, DELIVER_AT_LEAST_ONCE, (const uint8_t*)publishData, strlen(publishData), pub_least_ack_process, clientHandle);
...
int iothub_mqtt_client_run(void)
{
    ...
    else
    {
        ...
        IOTHUB_MQTT_CLIENT_HANDLE clientHandle = initialize_mqtt_client_handle(&options, endpoint, type, on_recv_callback, retryPolicy, retryTimeoutLimitInSeconds);
        ...
        subscribe[0].subscribeTopic = SUBTOPIC;
        ...
        const char* publishData = "$cmd=reboot&apikey=38f6a551132f623c48de&msgid=and 1=1 #&reslut=failed";

        //result = publish_mqtt_message(clientHandle, "/china/sh", DELIVER_EXACTLY_ONCE, (const uint8_t*)publishData, strlen(publishData), pub_least_ack_process , clientHandle);

        //if (result == __FAILURE__) {
        //    printf("Does not support DELIVER_EXACTLY_ONCE\r\n");
        //}

        //result = publish_mqtt_message(clientHandle, "/china/bj", DELIVER_AT_MOST_ONCE, (const uint8_t*)publishData, strlen(publishData), pub_most_once_process , clientHandle);

        //result = publish_mqtt_message(clientHandle, "/china/bj", DELIVER_AT_MOST_ONCE, (const uint8_t*)publishData, strlen(publishData), NULL , NULL);

        //result = publish_mqtt_message(clientHandle, "/china/sh", DELIVER_AT_LEAST_ONCE, (const uint8_t*)publishData, strlen(publishData), pub_least_ack_process , clientHandle);
        ...
        do
        {
            ...
            {
                result = publish_mqtt_message(clientHandle, PUBTOPIC, DELIVER_AT_LEAST_ONCE, (const uint8_t*)publishData, strlen(publishData), NULL , clientHandle);
